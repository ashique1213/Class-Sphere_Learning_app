...
import { fetchClassroom, joinClass } from "../../../api/classroomapi";
...
const Classroom = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const isStudentJoined = classroomData.students?.some(
        (student) => student.username === user?.username
      );
      setIsJoined(isStudentJoined);

      if (isStudentJoined) {
        const [meetingsData, examsData, submissionsData, materialsData, assignmentsData] = await Promise.all([
          fetchMeetings(slug),
          fetchExams(slug),
  ...
  const handleJoinClass = async () => {
    try {
      await joinClass(slug, authToken);
      setIsJoined(true);
      setShowJoinModal(false);
      toast.success("Class joined successfully!");
      navigate(`/classrooms/${user?.username}`);
    } catch (error) {
