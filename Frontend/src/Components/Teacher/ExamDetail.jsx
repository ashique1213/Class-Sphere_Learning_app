...
const ExamDetail = () => {
  ...
  const handleViewSubmissions = async () => {
    try {
      ...
      console.log("Show Submissions Set to True, Submissions:", examSubmissions);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <p className="text-center text-gray-600 text-lg">Loading exam details...</p>
      </div>
    );
  }
  ...
  const currentQuestions = exam.questions.slice(indexOfFirstQuestion, indexOfLastQuestion);
        ...
        <div className="max-w-5xl mx-auto bg-white p-6 rounded-xl shadow-lg">
          ...
          {showSubmissions && (
            <div className="mt-6 bg-gray-100 p-4 mb-4 rounded-lg shadow-sm">
              <h3 className="text-md font-semibold text-gray-900 mb-4">📋 Student Submissions</h3>
          ...
          <p className="text-gray-600 mb-2">
            <strong className="text-gray-800">Description:</strong> {exam.description}
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-gray-700 mb-6">
            <p>
              <strong className="text-gray-800">⏳ Timeout:</strong> {exam.timeout}
            </p>
            <p>
              <strong className="text-gray-800">📅 End Date:</strong> {exam.end_date}
            </p>
            <p>
          ...
          <h3 className="text-md font-semibold text-gray-900 mt-6">📌 Questions</h3>
            ...
            <>
              <div className="space-y-4 mt-4">
                {currentQuestions.map((q, index) => (
                  <div key={q.id || index} className="bg-gray-100 p-4 rounded-lg shadow-sm">
          ...
          <Link
            to={`/exams/${slug}`}
            className="mt-6 inline-block text-white bg-teal-500 px-4 py-2 rounded-lg hover:bg-teal-600 transition"
          >
            ← Back to Exams
          </Link>
        </div>
